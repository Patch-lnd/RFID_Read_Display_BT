...
SoftwareSerial mySerial(6, 5); // RX (Arduino) ← TX (module), TX (Arduino) → RX (module)
...
void loop()
{
  ...
  */
  ...
  //----------------------------------------------------------------------------------------------------
// Envoie des données lues au smartphone
  mySerial.println("CARD DATA:");
  for (int j = 0 ; j < 16 ; j++) {
    mySerial.write(readBlockData[j]);
  }
  mySerial.println();  // retour à la ligne

  // Affichage message d'attente
  delay(3000);
  lcd.clear();
  lcd.setCursor(0,0); lcd.print("Confirmation");
  lcd.setCursor(0,1); lcd.print("en cours...");

  // Attente de la réponse "ok" du smartphone
  String response = "";
  unsigned long startTime = millis();
  while (millis() - startTime < 10000) {  // timeout de 10s
    if (mySerial.available()) {
      char c = mySerial.read();
      response += c;
      if (response.endsWith("ok")) break;
    }
  }

  if (response.endsWith("ok")) {
    lcd.clear();
    lcd.setCursor(1,0); lcd.print("Operation");
    lcd.setCursor(2,1); lcd.print("Success");
  } else {
    lcd.clear();
